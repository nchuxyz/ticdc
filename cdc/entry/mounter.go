...
func (m *mounterImpl) unmarshalAndMountRowChanged(ctx context.Context, raw *model.RawKVEntry, rowKVEntryCache map[int64]*rowKVEntry) (*model.RowChangedEvent, error) {
	...
	row, err := func() (*model.RowChangedEvent, error) {
		...
		if bytes.HasPrefix(key, recordPrefix) {
			rowKV, err := m.unmarshalRowKVEntry(tableInfo, key, raw.Value, raw.OldValue, cachedRow)
			if err != nil {
				return nil, errors.Trace(err)
			}
			if rowKV == nil {
...
func (m *mounterImpl) unmarshalRowKVEntry(tableInfo *model.TableInfo, restKey []byte, rawValue []byte, rawOldValue []byte, cachedRow *rowKVEntry) (*rowKVEntry, error) {
	...
	if cachedRow.Delete && !m.enableOldValue && tableInfo.PKIsHandle {
